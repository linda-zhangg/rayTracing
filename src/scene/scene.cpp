...
Scene Scene::shapeScene() {
    ...
    objects.push_back(make_shared<SceneObject>(make_shared<Plane>(vec3(0,-1,0), normalize(vec3(0, -1, 0))),white));
    objects.push_back(make_shared<SceneObject>(make_shared<Disk>(vec3(0.5, 0, -5), normalize(vec3(1, 0, 1)), 0.6),white));
    objects.push_back(make_shared<SceneObject>(make_shared<Triangle>(vec3(2,-0.5,-5), vec3(2,1,-5),vec3(3,-0.5,-6)),white));
